...
```yaml
# POC/Prototype skills activation - speed over perfection
skills_context:
  design_rapid: # REQUIRED for polished POCs
    required:
      - ui-ux-pro-max             # Quick design decisions, palettes, pairings
      - frontend-design           # Non-generic, production-feel design
    recommended:
      - tailwind-design-system    # Rapid component styling
      - icon-design               # Quick semantic icon selection
      - better-icons              # Fast icon library search

  react_core: # REQUIRED
    required:
      - react-best-practices      # Correct patterns from the start

  typography_quick: # For stakeholder-ready POCs
    recommended:
      - font-pairing-suggester   # Quick font decisions
      - typography                # Basic hierarchy setup

  content: # For realistic-looking POCs
    recommended:
      - human-writing             # Natural placeholder copy

  poc_design_rules: # ALWAYS enforce
    hero_first: "Design hero section first - everything inherits its style"
    style_reference: "Use screenshot references, not text descriptions"
    icon_system: "Declare upfront: iconify solar duotone + SVG logos"
    avoid: ["purple gradients", "over-rounded corners", "generic SaaS look"]
```
...
framework: [React 19/Next.js 16/Vite]
